...
```rust,ignore
// This example implements a datasource `DataService` and requires the
// `data` (and `stream`) features: `cargo run --features data,stream`.
...
pub use grafana_config::{ConfigError, FeatureToggles, GrafanaConfig};
pub use instance::{
    app_instance_key, app_needs_update, Instance, InstanceManager, InstanceProvider,
};
pub use resource::{
...
// The `Plugin` builder always carries type parameters for the data (`Q`) and stream
// (`S`) services so its public shape is stable across feature sets. When a feature is
// disabled the corresponding service trait does not exist, so these alias traits stand
// in for the `Plugin::start` bounds: they resolve to the real service trait when the
// feature is on, and to a blanket-implemented marker (satisfied by `NoopService`) when
// it is off.
#[cfg(feature = "data")]
use data::DataService as MaybeDataService;
#[cfg(not(feature = "data"))]
#[doc(hidden)]
pub trait MaybeDataService {}
#[cfg(not(feature = "data"))]
impl<T> MaybeDataService for T {}

#[cfg(feature = "stream")]
use stream::StreamService as MaybeStreamService;
...
/// as [`Plugin::resource_service`] and [`Plugin::diagnostics_service`], or
/// `Plugin::data_service`/`Plugin::stream_service` with the `data`/`stream`
/// features) will return a new `Plugin` with your plugin implementation registered
...
///
/// ```rust,ignore
/// // Requires the `data` and `stream` features.
/// use futures_util::stream::FuturesOrdered;
    ...
    S: MaybeStreamService + Send + Sync + 'static,
{
    ...
    pub async fn start(self, listener: TcpListener) -> Result<(), Error> {
        ...
        #[cfg(feature = "data")]
        {
            router = router.add_optional_service(self.data_service.map(DataServer::new));
        }
        #[cfg(feature = "stream")]
        {
        ...
        if let Some(handler) = self.shutdown_handler {
            ...
            if let Some(config) = tls {
                router
                    .serve_with_incoming(mtls::tls_incoming(listener, config))
                    .await?;
                return Ok(());
            }
            router
...
/// # Automatic mTLS
///
/// When the `automtls` feature is enabled and Grafana requests automatic mTLS (by
/// setting the `PLUGIN_CLIENT_CERT` environment variable), this generates a server
/// certificate, prepares a mutually-authenticated TLS configuration for
/// [`Plugin::start`] to serve with, and advertises the server certificate in the
/// handshake line. Otherwise the plugin serves plaintext gRPC.
///
...
pub async fn initialize() -> Result<TcpListener, io::Error> {
    ...
    #[cfg(feature = "automtls")]
    {
        match mtls::server_tls_from_env() {
            Ok(Some((config, server_cert))) => {
                let _ = mtls::SERVER_TLS.set(Some(config));
                // The sixth handshake field is the server certificate; the client
                // uses it as the trust anchor for our TLS connection.
                println!("1|2|tcp|{address}|grpc|{server_cert}");
            }
            Ok(None) => {
                let _ = mtls::SERVER_TLS.set(None);
                println!("1|2|tcp|{address}|grpc");
            }
            Err(e) => {
                return Err(io::Error::other(format!(
                    "failed to configure automatic mTLS: {e}"
                )));
            }
        }
    }
    #[cfg(not(feature = "automtls"))]
    {
        if std::env::var_os("PLUGIN_CLIENT_CERT").is_some() {
            eprintln!(
                "warning: Grafana requested automatic mTLS (PLUGIN_CLIENT_CERT is set) but this \
                 plugin was built without the `automtls` feature; the connection will fail. \
                 Rebuild with the `automtls` feature enabled."
            );
        }
        println!("1|2|tcp|{address}|grpc");
    }
